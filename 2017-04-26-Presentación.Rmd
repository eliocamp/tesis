...
map.world.3 <- BuildMap(res = 3, smooth = 3, pm = 0)
...
ggplot(qs_index, aes(month(date), amplitude.norm, group = month(date))) +
    geom_boxplot() +
    ylab("Amplitud normalizada") + scale_x_continuous(breaks = 1:12, labels = month.abb)
...
gh.composition <- gh[, .(gh = mean(gh[amplitude.norm >= 1]) - mean(gh)), 
                     by = .(lon, lat, lev, month(date))]
```
