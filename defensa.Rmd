...
AddPreSuf <- function(prefix = "", suffix = "") {
    function(string) {
...
lev.lab <- c("Tropósfera media\n500hPa",
             "Tropósfera alta\n200hPa",
             "Estratósfera\n50hPa")
lev.lab <- setNames(lev.lab, c("500", "200", "50"))

# Plot thingys
guide_colorstrip_bottom <- function(width = 35, height = 0.5, ...) {
...
copy(ncep[lev == 700 & lat <= -20]) %>% 
    .[, gh.t := Anomaly(gh), by = .(lon, lat, month(date))] %>% 
    .[, gh.w := gh.t*sqrt(cos(lat*pi/180))] %>%
    EOF(lon + lat ~ date, value.var = "gh.w", n = 1) -> aao

ggplot(RepeatLon(aao$left), aes(lon, lat)) +
    geom_contour_fill(aes(z = value*aao$sdev$sd[1]/10), binwidth = 10,
...
```{r}
plot.levs <- c(500, 200, 50)
```


```{r, fig.class = "fullpage", fig.cap = "Z (mgp). Contornos cada 250 mgp (NCEP)."}
binwidth <- 250
ncep.mean.season[, lev2 := factor(lev, levels = c(500, 200, 50), 
                                  labels = c("Tropósfera media \n 500hPa",
                                             "Tropósfera alta \n 200hPa",
                                             "Estratósfera \n 50hPa"),
                                  ordered = TRUE)]
ggplot(ncep.mean.season[lev %in% plot.levs], aes(lon, lat)) +
    ...
    geom_label_contour2(aes(z = gh, label = ..level..), binwidth = binwidth,
                        size = 1.6) +
    scale_s_map() +
    facet_grid(season ~ lev2) +
    coord_quickmap()
```


```{r, fig.class = "fullpage", fig.cap = "Z* (mgp) (NCEP)."}
binwidth <- 20
cutlat <- -60
...
```
```{r}
ncep.mean.season[, lev2 := NULL]
```


```{r , fig.class = "pagewidth", fig.cap = "Corte zonal de Z* (mgp) en 60°S (NCEP)."}
binwidth <- 60
...
```{r, fig.publish = FALSE, fig.cap = "Z* menos QS1."}
...
```{r,  fig.class = "fullpage", fig.cap = "Función corriente media en $\\sigma = 0,2101$ (contornos cada $2\\times10^{-11}m^2/s$), anomalía zonal de función corriente (sombreado,  $1\\times10^{-9}m^2/s$) y flujos de actividad de onda medios (NCEP)."}
library(scales)
...
```{r, fig.class = "fullpage", fig.cap = "$r^2$ de Fourier para números de onda 1 a 4 (NCEP)."}
binwidth <- 0.1
ggplot(ncep.qs, aes(lat, lev)) +
    ...
    scale_x_latitude(name = "latitud", trans = "reverse", 
                     ticks = 15) +
    coord_latlev() +
    facet_grid(season ~ k, labeller = labeller(k = qs.lab))
```


```{r, fig.class = "fullpage", fig.cap = "Amplitud de Fourier (mgp) para números de onda 1 a 4 (NCEP)."}
breaks <- 2^seq(0, log2(650), by = 1)
ggplot(ncep.qs, aes(lat, lev)) +
    stat_contour_fill(aes(z = amplitude), 
                      breaks = breaks, na.rm = T) +
    surface +
    geom_index.region(rect.annotation) +
...
```{r, fig.class = "pagewidth", fig.cap = "Z* media (mgp) reconstruida a partir de la QS3 en 300hPa (NCEP).", fig.height = 6}
binwidth <- 10 
...
```{r, fig.class = "pagewidth", fig.cap = "Corte zonal en 60°S de Z* (mgp) reconstruida a partir de la QS3 (NCEP)."}
...
```{r, fig.cap = "Anomalía zonal de la amplitud de la QS3 según wavelets (mpg) en 300hPa para el método AM y MA (\\autoref{metodologia}) (NCEP).", fig.class = "vfullpage"}
copy(wv.mean.season) %>% 
...
```{r}
# wv.mean.season <-  ncep.mean[, lapply(.SD, mean), 
#                               by = .(lon, lat, lev, qs.season(month))] %>% 
#     .[,  .(lon = lon,
#            amplitude = unlist(PeriodicWavelet(gh, 3))),
#       by = .(lat, lev, qs.season)]
# 
# file.wavelet <- "DATA/ncep.wv.Rds"
# wv.all <- cache.file(file.wavelet, {
#     ncep[,  .(lon = lon, gh.z = gh.z,
...
# wv.mean.season <- wv.mean.season[wv.mean.season2, 
#                                  on = c("lat", "lon", "qs.season", "lev")]
binwidth <- 0.1
copy(wv.mean.season) %>% 
...
```{r, fig.cap = "Amplitud de la QS3 máxima y media (mgp) para 9 casos seleccionados.", fig.height = 4, fig.class = "pagewidth"}
...
```{r, fig.cap = "Corte vertical de amplitud de la QS3 (mgp) para 9 casos seleccionados.", fig.class = "pagewidth", fig.height = 4}
...
```{r, fig.cap = "Z* (mgp) en 300hPa con QS1 y QS2 eliminadas para 9 casos seleccionados.", fig.class = "pagewidth", fig.height = 6}
lons <- unique(ncep$lon)
...
```{r, fig.cap = "Primeras dos componentes principales del campo de Z* reconstruido a partir de la QS3."}
...
```{r, fig.cap = 'Valor medio de las dos primeras componentes principales del campo de Z* reconstruido a partir de la QS3 para cada mes. Las líneas unen cada mes siguiendo el orden anual y los colores separan a las 5 "estaciones" definidas en el texto.', fig.height = 5}
...
```{r, fig.class = "pagewidth", fig.cap = "Z* media reconstruida a partir de la QS3 en 300hPa (mgp) según las estaciones definidas en el texto.", fig.height = 6}
qs.qs.season <- qs.wave[, 
                        .(QS3 = mean(QS3)),
                        by = .(lon, lat, lev, qs.season(date))] 

binwidth <- 10
ggplot(qs.qs.season[lev == 300 & qs.season != "ND"], aes(lon, lat)) +
...
```{r, fig.class = "pagewidth", fig.cap = "Corte en 52,5°S de Z* media reconstruida a partir de la QS3 en 300hPa (mgp) según las estaciones definidas en el texto."}
ggplot(qs.qs.season[lat %~% -52.5 & qs.season != "ND"], aes(lon, lev)) +
    # stat_contour_fill(aes(z = sd), binwidth = 10) +
...
```{r, fig.cap = "Regresión de Z en 300hPa con $A_3$ estandarizado (mgp).", fig.class = "fullpage"}
binwidth <- 10
months <- data.frame(month = 1:12, l = month.abb_sp, lon = 180, lat = -10)
regression[regressor == "amplitude"] %>% 
    ...
    geom_label(data = months, aes(label = l), label.r = unit(0, "lines"),
               color = "white") +
    geom_text(data = months, aes(label = l)) +
...
```{r, fig.cap = "Regresión de la anomalía zonal de $\\psi$ en $\\sigma = 0,2101$ con $A_3$ estandarizado ($1\\times10^{-6}m^2/s$) y flujos de actividad de onda calculados a partir de la misma.", fig.class = "fullpage"}
binwidth <- 1
scale <- 600
...
```{r, fig.class = "fullpage", fig.cap = "Z* (mgp) (SPEEDY sombreado, NCEP contornos)."}
plot.levs.sp <- c(500, 200)
binwidth <- 20
cutlat <- -60

speedy.mean.season[, lev2 := factor(lev, levels = c(500, 200), 
                                  labels = c("Tropósfera media \n 500hPa",
                                             "Tropósfera alta \n 200hPa"),
                                  ordered = TRUE)]
ncep.mean.season[, lev2 := factor(lev, levels = c(500, 200), 
                                  labels = c("Tropósfera media \n 500hPa",
                                             "Tropósfera alta \n 200hPa"),
                                  ordered = TRUE)]
...
```{r, fig.class = "pagewidth", fig.cap = "Corte zonal de Z* (mgp) en 60°S (SPEEDY sombreado, NCEP contornos)."}
binwidth <- 20
...
```{r, fig.class = "pagewidth", fig.cap = "Media zonal del viento zonal (m/s) (SPEEDY sombreado, NCEP contornos)."}
...
```{r, fig.class = "pagewidth", fig.cap = "Amplitud de la QS3 a partir de Fourier (mgp) (SPEEDY sombreado, NCEP contornos)."}
...
qs.wave.sp.season <- speedy[, .(mean = mean(QS3)), 
                            by = .(lon, lat, season(date), run, lev)]
```


```{r, fig.class = "pagewidth", fig.cap = "Z* reconstruida a partir de la QS3 (mgp) (SPEEDY sombreado, NCEP contornos).", fig.height = 6}
binwidth <- 10
cutlat <- data.frame(lat = c(-52.5), 
                     season = c("Verano"))
ggplot(qs.wave.sp.season[run == "Control" & lev == 300], aes(lon, lat)) +
...
```{r}
binwidth <- 10
ggplot(qs.wave.sp.season[lat %~% cutlat$lat[1] & run == "Control"], aes(lon, lev)) +
    # stat_contour_fill(aes(z = sd), binwidth = 10) +
    geom_contour_fill(aes(z = mean), binwidth = binwidth, exclude = 0) +
    # geom_path(data = qs.wave.season[lon < 90 & lat %~% cutlat$lat[1] & mean > 5, 
    #            .(lon = lon[which.max(mean)]), 
...
```{r, fig.class = "fullpage", fig.cap = "Z* en 200hPa para cada corrida de SPEEDY (mgp)."}
plot.levs.sp <- 200
binwidth <- 20
...
ggplot(speedy.mean.season[lev == 200], aes(lon, lat)) +
    ...
    scale_s_map() +
    coord_quickmap() +
    facet_grid(season ~ run, labeller = labeller(lev = lev.lab)) 
```
...
```{r, fig.cap = "Amplitud de la QS3 (mgp) para cada corrida según Fourier y método AM.", fig.class = "fullpage"}
...
```{r, fig.cap = "Amplitud de la QS3 (mgp) para cada corrida según Fourier y método MA.", fig.class = "fullpage"}
...
```{r , fig.cap = "Desvío estándar (en grados) de la fase media mensual para cada estación y cada corrida.", fig.height = 4}
