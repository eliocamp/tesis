...
# 
# # v't' 
# ncep.vt <- ncep.d[, c("t.t", "v.t") := .(Anomaly(air), Anomaly(vwnd)), by = .(lon, lat, level)][
#     , .(vt = mean(t.t*v.t)), by = .(lon, lat, level, year(date))]
# 
# remove(ncep.d, ncep_d.v850)
# 
...
# 
# setnames(ncep.d, c("level", "vwnd", "uwnd", "air"), c("lev", "v", "u", "t"))
# 
# # Valores mensuales
...
# a <- 6371000
# omega <- 2*pi/(24*3600)
# ncep[, v.dx := Derivate(v, lon)/(a*cos(lat*pi/180)), by = .(lat, lev, year)][
#      , u.dy := Derivate(u, lat, bc = "none")/a, by = .(lon, lev, year)][
#      , zeta := v.dx - u.dy][
