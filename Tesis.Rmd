...
```{r cor-field, fig.cap = "Media anual de la correlación cuadrada de cada mes entre el campo observado y el reconstruido."}
cor.mean <- fields[lat > -90 & lev == 300, 
...
cor.mean <- fields[lat > -90 & lev == 300, 
                           .(corelation = cor(gh.z, QS3)^2), 
                           by = .(lon, lat)]
ggplot(cor.mean, aes(lon, lat)) +
    stat_contour(aes(z = corelation, color = ..level..), binwidth = 0.15) +
    map.SH +
    scale_color_viridis_c(breaks = MakeBreaks(0.15), 
                          guide = guide_colorbar_bottom()) +
    scale_s_map() +
    coord_quickmap()
```
...
PlotComposition <- function(field, binwidth = binwidth) {
    g1 <-
        g1
}
```
...
```{r select-enero, fig.cap = "Campos para los 10 eneros seleccionados.", fig.height = 11}
tempplot(1)
```

```{r select-sept, fig.cap = "Campos para los 10 septiembres seleccionados.", fig.height = 11}
tempplot(9)
```
...
Cosas para ver:    
La mayoría del tiempo estamos afectados por centro de máxima, pero hay casos intensos donde hay mínima. 
