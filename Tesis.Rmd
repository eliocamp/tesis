...
```{r cor-field, fig.cap = "Correlación cuadrada media para estaciones según onda3.", fig.show = "hold", fig.ncol = 2}
...
tempplot <- function(month) {
    ggplot(mapping = aes(lon, lat, z = gh.z)) +
        stat_contour_fill(data = gdata[campo == "qs3" & month(date) == month], 
                          exclude = 0, complete = F,
                          binwidth = binwidth) +
        geom_contour_fine(data = gdata[campo == "ncep" & month(date) == month],
...
```{r eof-comp, fig.class = "fullpage", fig.cap = "Campos EOF para cada mes (probablemente basura)"}
...
ggplot(eofs[campo == "ncep"], aes(lon, lat)) +
...
```{r sam-ampl, fig.cap = "Relación entre amplitud media de la onda 3 y el SAM"}
...
ggplot(fields[campo == "qs3" & date %in% dates], 
       aes(lon, lat)) +
    stat_contour_fill(aes(z = gh.z), exclude = 0, complete = F,
                      binwidth = 10) +
