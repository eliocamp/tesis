...
binwidth <- 1
ggplot(RepeatLon(ncep.mean.season[lev %in% c(plot.levs, 850)]), 
       aes(lon, lat)) +
    stat_contour_fill(aes(z = t.z), binwidth = binwidth, exclude = 0) +
    geom_hline(yintercept = cutlat, linetype = 2, size = 0.5) +
    map.SH +
    scale_fill_divergent(binwidth = binwidth, name = "T*", 
                         guide = guide_colorstrip_bottom(25)) +
    scale_y_latitude() +
    scale_x_longitude() +
    facet_grid(lev ~ season, labeller = labeller(lev = lev.lab)) +
...
```{r sp-nc-regr, fig.cap = "Correlación lineal entre campos de SPEEDY y NCEP.", fig.height = 5, fig.publish = FALSE}
...
```{r u-nce-sp-dif, fig.cap = "Diferencia entre ncep y speedy en viento zonal"}
binwidth <- 5
...
```{r sp-nc-psi,  fig.class = "fullpage", fig.cap = "Función corriente x 1099"}
...
```{r sp-corridas.gh.z, fig.class = "fullpage", fig.cap = "Anomalía zonal de altura geopotencial (speedy sombreado, ncep contornos)"}
...
```{r dif-gh.z, fig.cap = "Diferencia Control - corrida para Z*"}
...
run.dif <- setNames(paste0("Control -\n", runs[-1]),
                runs[-1])
binwidth <- 10
speedy.mean.season.l[lev == 200 & variable == "gh.z" & run != "Control"] %>% 
ggplot(aes(lon, lat, z = dif)) +
    geom_contour_fill(binwidth = binwidth) +
    map.SH +
    scale_s_map() +
    scale_fill_divergent(breaks = MakeBreaks(binwidth),
...
No hay casi diferencia entre las corridas. 

```{r sp-corridas.t.z, fig.class = "fullpage", fig.cap = "Anomalía zonal de altura geopotencial (speedy sombreado, ncep contornos)"}
binwidth <- 5
ggplot(speedy.mean.season[lev == 850], aes(lon, lat)) +
    geom_contour_fill(aes(z = t), 
                      binwidth = binwidth, complete = F, exclude = 0) +
    scale_fill_divergent(binwidth = binwidth, name = "Z*", 
                         guide = guide_colorstrip_bottom()) +
    scale_linetype(guide = "none") +
    map.SH +
...
```{r sp-corridas.u.z, fig.class = "fullpage", fig.cap = "Anomalía zonal de altura geopotencial (speedy sombreado, ncep contornos)"}
binwidth <- 5
ggplot(speedy.mean.season[lev == 200], aes(lon, lat)) +
...
```{r dif-u, fig.cap = "Diferencia control - corrida para U."}
binwidth <- 2
speedy.mean.season[lev == 200] %>% 
...
ggplot(aes(lon, lat)) +
    ...
    scale_x_longitude() +
    coord_quickmap() +
    facet_grid(run ~ season, labeller = labeller(run = dif)) 
```

### Función corriente

```{r sp-corridas.psi, fig.class = "fullpage", fig.cap = "Anomalía zonal de altura geopotencial (speedy sombreado, ncep contornos)"}
binwidth <- 2
ggplot(speedy.mean.season[lev == 200], aes(lon, lat)) +
