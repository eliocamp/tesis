...
gdata <- fields[date %in% select.dates & month(date) == c(1, 9)] %>% 
    .[, year := year(date), by = date] %>% 
    enso[., on = "date"] 

binwidth <- 30
tempplot <- function(month) {
    ggplot(mapping = aes(lon, lat, z = gh)) +
        stat_contour_fill(data = gdata[campo == "qs3" & month(date) == month], 
                          exclude = 0, complete = F,
                          binwidth = binwidth) +
        geom_contour_fine(data = gdata[campo == "ncep.z" & month(date) == month],
                          aes(linetype = factor(sign(-..level..))),
                          binwidth = binwidth,
                          color = "black") +
        geom_point(data = gdata[lon %~% 15 & lat %~% -5], 
                   aes(color = sign)) +
        map.SH +
        scale_s_map() +
...
speedy <- ReadNetCDF("DATA/SPEEDY/attm106.nc", vars = vars)
speedy <- speedy[lat <= 0]
speedy[, t := t - 273.15]
...
```{r sp-nc-regr, fig.cap = "Correlación lineal entre campos de SPEEDY y NCEP."}
...
```{r sp-nc-u-corte, fig.class = "textwidth", fig.cap = "Viento zonal medio."}
...
```{r u_medio_sp, fig.class = "fullpage", fig.cap = "Viento zonal."}
ggplot(speedy.mean.season[lev %in% plot.levs], aes(lon, lat)) +
...
```{r u_nomalia_sp, fig.class = "fullpage", fig.cap = "Anomalía zonal de viento zonal."}
binwidth <- 2.5
ggplot(speedy.mean.season[lev %in% plot.levs], aes(lon, lat)) +
    stat_contour_fill(aes(z = u.z), binwidth = binwidth, exclude = 0) +
    # geom_hline(yintercept = cutlat, linetype = 2, size = 0.5) +
...
```{r eta-dy-sp-nc, fig.class = "fullpage", fig.cap = "Gradiente meridional de vorticidad absoluta."}
binwidth <- 1e-11
...
```{r k-steady-sp, fig.cap = "Número de onda estacionario en 300hPa.", fig.height = 6}
a <- 6371000
...
```{r read-corridas}
gh.sp <- readRDS("DATA/SPEEDY/speedy.Rds") %>% 
    .[, .(lat, lon, lev, date, gh)] %>% 
    .[, run := "Control"]
gh.sp.clim <- readRDS("DATA/SPEEDY/speedy.clim.Rds") %>% 
    .[, .(lat, lon, lev, date, gh)] %>% 
    .[, run := "SSTCLIM"]
gh.sp.noice <- readRDS("DATA/SPEEDY/speedy.noice.Rds") %>% 
    .[, .(lat, lon, lev, date, gh)] %>% 
    .[, run := "NOICE"]
gh.sp.noland <- readRDS("DATA/SPEEDY/speedy.noland.Rds") %>% 
    .[, .(lat, lon, lev, date, gh)] %>% 
    .[, run := "NOLAND"]

gh.sp <- rbind(gh.sp, gh.sp.clim, gh.sp.noice, gh.sp.noland)
remove(gh.sp.clim, gh.sp.noice, gh.sp.noland)
gh.sp[, run := factor(run, levels = c("Control", "SSTCLIM", 
                                      "NOICE", "NOLAND"))]
# Calculo la onda 3
qs.sp <- gh.sp[, FitQsWave(gh, 3), by = .(lat, lev, date, run)]
qs.sp.season <- qs.sp[, lapply(.SD, mean), by = .(lat, lev, season(date), run)]

levs.index <- c(700, 300, 100)
```

```{r ampl-corridas, fig.cap = c("Amplitud media de la onda 3 para cada corrida.")}
sp.levs <- unique(speedy$lev)
ggplot(qs.sp.season, aes(lat, lev)) +
    geom_contour_fill(aes(z = amplitude), binwidth = 10) +
    scale_x_latitude(trans = "reverse") +
...
```{r control-run, fig.cap = "Diferencia de amplitud entre la corrida control y cada corrida."}
