...
ggplot(ncep.mean.season[lev %in% c(plot.levs, 850)], aes(lon, lat)) +
    ...
    map.SH +
    scale_s_map() +
    # scale_color_viridis_c(name = "T", guide = guide_colorstrip_bottom(25), 
    # breaks = MakeBreaks(binwidth)) +
    coord_quickmap() +
    facet_grid(lev~season, labeller = labeller(lev = lev.lab))
```
...
binwidth <- 20
cutlat <- -60
ggplot(speedy.mean.season[lev %in% plot.levs.sp], aes(lon, lat)) +
    ...
    coord_quickmap()
```
```{r sp-nc-dif-gh.z, fig.class = "fullpage", fig.cap = "Diferencia entre speedy y ncep"}
binwidth <- 20
...
ggplot(RepeatLon(speedy.mean.season[lev %in% plot.levs.sp]), aes(lon, lat)) +
    geom_contour_fill(aes(z = u), binwidth = 5) +
    geom_contour_back(aes(z = u, linetype = factor(-sign(..level..))), 
                      binwidth = 5, 
                      data = RepeatLon(ncep.mean.season[lev %in% plot.levs.sp])) +
    geom_label_contour_back(aes(z = u, label = ..level..), 
                            binwidth = 5, 
                            data = RepeatLon(ncep.mean.season[lev %in% plot.levs.sp])) +
    # geom_contour(aes(z = u, color = ..level..), binwidth = 5, size = 0.1) +
...
binwidth <- 1e-11
ggplot(speedy.mean.season[lev %in% plot.levs.sp & lat > -85], aes(lon, lat)) +
    stat_contour_fill(aes(z = eta.dy), binwidth = binwidth) +
    stat_contour_fill(aes(z = eta.dy), breaks = 0, 
                      fill = "grey50", alpha = 0.5, complete = FALSE) +
    geom_contour_fine(aes(z = eta.dy), breaks = 0, color = "black") +
...
speedy <- speedy[, c("amplitude", "phase", "r2") := FitQsWave(gh, 3),
                 by = .(lat, lev, date)]
...
cutlat <- data.frame(lat = c(-52.5), 
                     season = c("Verano"))
ggplot(qs.wave.sp.season, aes(lon, lat)) +
    # stat_contour_fill(aes(z = sd), binwidth = 10) +
...
runs <- c("Control", "SSTCLIM", "NOICE", "NOLAND", "SSTZONAL") 
...
# qs.sp.season <- speedy[, mean(gh), 
                       # by = .(lon, lat, lev, run, season(date))] %>% 
    # .[, FitQsWave(V1, 3), by = .(lat, lev, run, season)]
