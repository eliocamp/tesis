...
pres.mean <- pres[, .(pres = mean(pres)), by = lat]
pres.mean <- rbind(data.table(lat = 0.0, pres = Inf), 
                   pres.mean, 
                   data.table(lat = -90.0, pres = Inf))
surface <- geom_polygon(data = pres.mean, aes(y = pres), fill = "gray40", 
...
ggplot(RepeatLon(ncep.mean.season[lat %~% cutlat]), aes(lon, lev)) +
    stat_contour_fill(aes(z = u.z), binwidth = binwidth, exclude = 0) +
    scale_fill_divergent(binwidth = binwidth, exclude = 0, name = "U*",
                         guide = guide_colorbar_bottom(width = 25)) +
    scale_y_level(breaks = plot.levs, minor_breaks = NULL) +
    scale_x_longitude(name = "longitud") +
    facet_wrap(~season)
...
```{r eta_calc_ncep, fig.class = "halfpage"}
ncep.mean.season %>% 
    .[, `:=`("v.dx" = Derivate(v ~ lon + lat, bc = c("cyclic", "none"), 
                               sphere = TRUE)[[1]],
...
```{r r2_qs_ncep}
binwidth <- 0.1
rect.annotation <- data.frame(xmin = -55, xmax = -35,
                              ymax = Inf, ymin = 50,
                              k = c(1, 3))
