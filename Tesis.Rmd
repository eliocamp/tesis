...
qs.eof <- EOF(lon + lat ~ date, value.var = "QS3w", 
              data = fields[lev == 300][, QS3w := QS3*sqrt(cos(lat*pi/180))],
              n = 1:2)
qs.eof$left[PC == "PC2", QS3w := -QS3w]
qs.eof$right[PC == "PC2", QS3w := -QS3w]
...
ggplot(cor.season, aes(lon, lat)) +
    geom_contour_fill(aes(z = corelation), xwrap = c(0, 360),
                      breaks = MakeBreaks(binwidth)(c(0.1, 0.6)),
                      complete = FALSE) +
    geom_contour_fine(aes(z = mean, color = factor(sign(..level..))),
                      xwrap = c(0, 360),
...
binwidth <- 10
ggplot(regression[term == "amplitude" & month %in% c(7,12)], 
       aes(lon, lat)) +
    geom_contour_fill(aes(z = estimate), xwrap = c(0, 360),
                      breaks = AnchorBreaks(0, binwidth, 0)) +
    map.SH +
...
ggplot(stream.reg[term == "amplitude" & lat <= 0], 
       aes(lon, lat)) +
    geom_contour_fill(aes(z = estimate*1e-6), xwrap = c(0, 360),
                      breaks = AnchorBreaks(0, binwidth, 0)) +
...
ggplot(speedy.mean.season[run == "Control" & lev %in% plot.levs.sp],
       aes(lon, lat)) +
    geom_contour_fill(aes(z = gh.z), breaks = AnchorBreaks(0, binwidth, 0),
                      xwrap = c(0, 360)) +
    ...
    geom_label_contour_back(aes(z = gh.z), xwrap = c(0, 360),
                            breaks = AnchorBreaks(0, binwidth, 0),
                            data = ncep.mean.season[lev %in% plot.levs.sp]) +
    scale_fill_divergent(breaks = AnchorBreaks(0, binwidth, 0),
                         name = "Z*", 
...
cowplot::plot_grid({
    ggplot(speedy.mean.season[run == "Control" & lev %in% 200], 
           aes(lon, lat, z = t)) +
        geom_contour_fill(binwidth = binwidth, xwrap = c(0, 360)) +
        # geom_contour_fine(aes(z = gh.z), binwidth = binwidth, exclude = 0,
        # color = "black", size = 0.2) +
        geom_contour_back(aes(linetype = factor(sign(-..level..))),
                          binwidth = binwidth, xwrap = c(0, 360),
...
ggplot(psi.sp.season[run == "Control"], aes(lon, lat)) +
    geom_contour_fill(aes(z = psi.z*mult), xwrap = c(0, 360),
                      breaks = AnchorBreaks(0, binwidth, 0)) +
    geom_contour_fine(aes(z = psi*mult), xwrap = c(0, 360),
                      binwidth = 2,
                      color = "black") +
...
ggplot(qs.wave.sp.season[run == "Control"], aes(lon, lat)) +
    geom_contour_fill(aes(z = mean), breaks = AnchorBreaks(0, binwidth, 0),
                      na.rm = T, xwrap = c(0, 360)) +
    geom_contour_back(aes(z = mean, linetype = factor(-sign(..level..))),
                      binwidth = binwidth, xwrap = c(0, 360),
                      data = qs.wave.season[lev == 300]) +
    geom_label_contour_back(aes(z = mean), xwrap = c(0, 360),
                            binwidth = binwidth,
                            data = qs.wave.season[lev == 300]) +
    map.SH +
    scale_s_map() +
