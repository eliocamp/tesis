...
ncep.f <- memoise(function() {
    subset <- list(lat = -90:0, time = lubridate::as_datetime(c("1985-01-01",
                                                            "2017-12-01")))
...
ggplot(aes(lon, lat)) +
    ...
    geom_text_contour(aes(z = t), binwidth = binwidth, stroke = 0.2, 
                      rotate = FALSE, size = 4) +
...
ggplot(ncep.mean.season[lev == 200], aes(lon, lat)) +
    ...
    stat_subset(aes(subset = !is.finite(k)), geom = "tile", color = "gray50",
                fill = "gray50") +
    geom_contour_fine(aes(z = k), breaks = 0:9, color = "black") +
    geom_text_contour(aes(z = k), breaks = 0:9, skip = 0,
...
ggplot(ncep.mean.season[lev == 300], aes(lon, lat)) +
    geom_contour_fill(aes(z = wavelet3), breaks = AnchorBreaks(0, 25, 0)) +
    scale_fill_divergent() +
    map.SH +
    facet_wrap(~season)
```
...
ncep.qs.all <- ncep.qs.all[, index.region := (lev %between% levs.index) & (lat %between% lats.index)]
...
```{r ampl-max-mean-corte, fig.cap = "Corte vertical de amplitud de la QS3 (mgp) para 9 casos seleccionados. La región blanca representa la topografía media.", fig.class = "pagewidth", fig.height = 4}
...
```{r u-sp-nc-corte, fig.class = "pagewidth", fig.cap = "Media zonal del viento zonal (m/s) (SPEEDY sombreado, NCEP contornos). La región blanca representa la topografía media."}
...
```{r psi-sp, fig.class = "fullpage", fig.cap = "$\\psi$ media en 200hPa (contornos cada $2\\times10^{-7}m^2/s$), anomalía zonal de $\\psi$ (sombreado,  $0,25\\times10^{-7}m^2/s$) y flujos de actividad de onda medios."}
