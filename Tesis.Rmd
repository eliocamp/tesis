...
```{r make-reg-psi}
stream.reg <- stream[, .(lon, lat, date, psi.z)][
    index[, .(date, amplitude, phase)], on = "date"][
        , setNames(ExtractLm(
            RcppArmadillo::fastLm(cbind(mean = 1,
                                        amplitude = amplitude/sd(amplitude), 
...
stream.reg[, psi.z := estimate]
stream.reg[, c("f.x", "f.y") := WaveFlux(.SD), by = .(regressor, month)]
```

```{r psi-reg, fig.cap = "Regresión de Psi con la amplitud."}
binwidth <- 1e6
ggplot(RepeatLon(stream.reg[regressor == "amplitude"]), 
       aes(lon, lat)) +
    stat_contour_fill(aes(z = estimate), binwidth = binwidth, exclude = 0, 
                      complete = FALSE) +
        geom_arrow(aes(dx = f.x, dy = f.y), skip = 4, min.mag = 0.005, 
               arrow.size = 0.15, scale = 600) +
    map.world +
    # scale_s_map() +
