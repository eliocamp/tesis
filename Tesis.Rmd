...
```{r dif-t.z, fig.cap = "Diferencia Control - corrida para Z*"}
binwidth <- 1
speedy.mean.season.l[lev == 850 & variable == "t.z" & run != "Control"] %>% 
ggplot(aes(lon, lat, z = dif)) +
    geom_contour_fill(binwidth = binwidth, exclude = 0) +
    map.SH +
    scale_s_map() +
...
regr.exps[regressor == "amplitude"] %>% 
    .[, mean(estimate), by = .(lon, lat, season(month), run)] %>% 
    .[, V1.z := Anomaly(V1), by = .(lat, season, run)] %>% 
    # .[, c("fx", "fy") := WaveFlux(.SD), by = ]
ggplot(aes(lon, lat, z = V1.z)) +
    geom_contour_fill(binwidth = binwidth, exclude = 0) +
