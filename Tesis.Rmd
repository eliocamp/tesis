...
```{r corplot, cache = FALSE}
corplot <- function(field, binwidth = 0.1) {
    ggplot(field, aes(lon, lat)) +
    stat_contour(aes(z = corelation, color = ..level..), binwidth = binwidth) +
    map.SH +
...
```{r cor-field, fig.cap = "Media anual de la correlación cuadrada de cada mes entre el campo observado y el reconstruido.", fig.show = "hold", fig.ncol = 2}
cor.mean1.z <- fields[lat > -90 & lev == 300, 
                   .(corelation = cor(gh.z, QS3)^2), 
                   by = .(lon, lat, month(date))][
                       , .(corelation = mean(corelation)), 
                       by = .(lon, lat)]
cor.mean2.z <- fields[lat > -90 & lev == 300, 
                   .(corelation = cor(gh.z, QS3)^2), 
                   by = .(lon, lat)]

cor.mean1 <- fields[lat > -90 & lev == 300, 
                   .(corelation = cor(gh, QS3)^2), 
                   by = .(lon, lat, month(date))][
                       , .(corelation = mean(corelation)), 
                       by = .(lon, lat)]
cor.mean2 <- fields[lat > -90 & lev == 300, 
                   .(corelation = cor(gh, QS3)^2), 
                   by = .(lon, lat)]



corplot(cor.mean1.z) + labs(subtitle = "Correlación con Z* mensual promediada")
corplot(cor.mean2.z) + labs(subtitle = "Correlación con Z* total")
corplot(cor.mean1) + labs(subtitle = "Correlación con Z mensual promediada")
corplot(cor.mean2) + labs(subtitle = "Correlación con Z total")
```
