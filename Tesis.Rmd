...
En 300hPa, el gradiente meridional de temperatura en latitudes medias tiene un importante ciclo anual con máximo en invierno y mínimo en verano. En niveles inferiores, el ciclo anual es menos marcado y más dependiente de la latitud. En 500hPa, en 150°E, 30°S el máximo gradiente meridional de temperatura ocurre en invierno y el mínimo en verano, mientras que en 50°S el ciclo es el inverso.
...
ggplot(ncep.mean.season[lat %~% cutlat], aes(lon, lev)) +
    stat_contour_fill(aes(z = t.z), breaks = AnchorBreaks(0, binwidth, 0)) +
    scale_fill_divergent(breaks = AnchorBreaks(0, binwidth2, 0),
                         name = "T*",
                         guide = guide_colorstrip_bottom(width = 25)) +
    scale_y_level(breaks = c(1000, plot.levs, 10), minor_breaks = NULL) +
...
binwidth <- 2.5
ggplot(ncep.mean.season[lev %in% plot.levs], aes(lon, lat)) +
    stat_contour_fill(aes(z = u.z), breaks = AnchorBreaks(0, binwidth, 0)) +
    map.SH +
    scale_y_latitude() +
    scale_x_longitude() +
...
ggplot(ncep.mean.season[, .(v = mean(v)), by = .(lat, lev, season)],
       aes(lat, lev)) +
    stat_contour_fill(aes(z = v), breaks = AnchorBreaks(0, binwidth, 0)) +
    surface + 
...
ggplot(stream.mean.season,aes(lon, lat)) +
    stat_contour_fill(aes(z = psi.z*10^-9), 
...
ggplot(ncep.mean.season[lev %in% plot.levs & lat > -85], 
    ...
    geom_contour_fill(breaks = AnchorBreaks(0, binwidth)) +
...
ggplot(ncep.qs, aes(lat, lev)) +
    geom_contour_fill(aes(z = r2), binwidth = binwidth, na.rm = T) +
    surface +
...
cutlat <- -52.5
ggplot(qs.wave.season[lev == 300], aes(lon, lat)) +
    geom_contour_fill(aes(z = sd), binwidth = binwidth) +
    geom_contour(aes(z = mean), breaks = c(1, -1)*20, color = "black", 
                 size = 0.5) +
    map.SH +
    scale_s_map() +
...
wv.mean.season[, .(amplitude = mean(amplitude)), 
    ...
    geom_text_contour(aes(z = amplitude, label = ..level..), 
                      data = ncep.qs[k == 3], rotate = FALSE,
                      stroke = 0.1, stroke.color = "#452F73",
                      color = "white",
                      binwidth = binwidth) +
...
wv.comp[lev == 300] %>% 
    ggplot(aes(lon, lat)) +
    geom_contour_fill(aes(z = value.z), breaks = AnchorBreaks(0, 1, 0)) +
    map.SH +
    scale_s_map() +
...
ncep.qs.all <- ncep.qs.all[, index.region := (lev %between% levs.index) & 
                               (lat %between% lats.index)]
...
fields <- ncep[lev %between% levs.index, .(lon, lat, date, lev, gh, gh.z)] %>% 
    .[qs.wave[lev %between% levs.index], on = c("lat", "lon", "date", "lev")]
...
```{r ampl-max-mean-corte, fig.cap = "Corte vertical de amplitud de la QS3 (mgp) para 9 casos seleccionados. La línea punteada indica la región sobre la cual se toma el promedio o el máximo. La región blanca representa la topografía media.", fig.class = "pagewidth", fig.height = 4}
M <- max(gdata.index[date %in% select.dates, m.amplitude])
binwidth <- 15
ggplot(gdata, aes(lat, lev)) +
    geom_contour_fill(aes(z = amplitude), binwidth = binwidth) +
    geom_index.region(rect.annotation) +
    geom_segment(data = gdata.index[date %in% select.dates], 
                 inherit.aes = F,
                 size = 1, color = "#E41A1C",
    ...
    surface +
    scale_y_level(name = "nivel", breaks = c(1000, plot.levs, 10),
                  sec.axis = sec_axis(~-M/2*(log10(.) - 3), 
                                      name = "Amplitud")) +
    scale_x_latitude(name = "latitud", trans = "reverse") +
...
ncep[lev == 300 & date %in% select.dates, 
    ...
    .[, date.f := factor(as.character(date), 
                         levels = as.character(select.dates))] %>% 
    ggplot(aes(lon, lat)) +
    geom_contour_fill(aes(z = gh.minus), breaks = AnchorBreaks(0, 25, 0)) +
    map.SH + 
    scale_s_map() +
    scale_fill_divergent(breaks = AnchorBreaks(0, 25, 0),
                         guide = guide_colorstrip_bottom()) +
    coord_quickmap() +
    facet_wrap(~ date.f, labeller = labeller(date.f = labeller.date())) +
    theme(axis.title = element_blank()) 
...
ampl <- w$Ampl
dimnames(ampl) <- list(period = as.numeric(w$Period), 
                       date = as.character(index$date)) 
cowplot::plot_grid(
    {
    ...
    {
        with(w, 
             data.table(period = Period, power = Power.avg, 
                        pval = Power.avg.pval)) %>% 
            ggplot(aes(period, power)) +
            geom_line() +
            stat_subset(aes(subset = pval < 0.01), geom = "point") +
...
lat.lims <- c(-70, -22)
lon.lims <- c(240, 360)
map.arg <- geom_map2(BuildMap(countries = T)[lat %between% lat.lims & 
                                                 long %between% lon.lims],
                     color = "gray50")
nudge <- 0.54
size <- 2
...
copy(index) %>% 
    ...
    .[, mean(abs(interf) %between% (c(1/3, 2/3)*120*pi/180))*100, by = season(date)] %>% 
...
```{r eoftabla}
qs.eof <- EOF(lon + lat ~ date, value.var = "QS3w", 
              data = fields[lev == 300][, QS3w := QS3*sqrt(cos(lat*pi/180))],
              n = 1:5)
qs.eof$sdev[, .(`$r^2$` = r2)] %>% 
    # .[, PC := AddPreffix("PC ")(PC)] %>% 
    t() %>%
...
ggplot(qs.eof$left, aes(lon, lat)) + 
    geom_contour_fill(aes(z = QS3w), breaks = AnchorBreaks(0, 0.01, 0)) +
    map.SH +
...
qs.eof$right[, .(QS3w = mean(QS3w)), by = .(PC, month(date))] %>% 
    dcast(... ~ PC, value.var = "QS3w") %>% 
...
cor.season <- fields[lat > -90 & lev == 300, 
                     ...
                     by = .(lon, lat, season(date, lang = "es"))]

binwidth <- 0.1
ggplot(cor.season, aes(lon, lat)) +
    geom_contour_fill(aes(z = corelation), 
                      breaks = MakeBreaks(binwidth)) +
...
Los campos horizontales de $r^2$ en 300hPa se muestran en la \autoref{fig:cor-campo}. En las cuatro estaciones la QS3 explica la mayor parte de la varianza en el hemisferio occidental entre 60°S y 45°S. Lejos de ser homogéneos, los campos muestran tres máximos localizados con cierta persistencia durante el año. El primero, al sur del Índico, está presente en verano y otoño en 60°E que se encuentra más hacia el este en invierno y primavera. Algo similar sucede con el segundo máximo ubicado al sur del Pacífico, que se encuentra en 180° en otoño y en 120°O en primavera. Se observa un tercer máximo en el Atlántico sur cuya ubicación varía longitudinalmente poco. En verano y otoño se distingue un máximo en latitudes bajas en el pacífico central.

Si se compara la posición de los máximos de $r^2$ con los centros de QS3, en verano, el máximo del Índico, por ejemplo, coincide con un centro anómalamente anticiclónico en verano. No obstante, el máximo de $r^2$ de otoño sobre esa cuenca se encuentra entre dos centros de la onda y lo mismo pasa con el máximo del Atlántico. Éste último coincide con un centro de anomalía anticiclónica en verano, aunque en invierno está más cerca de uno ciclónico. En suma, no parece haber una asociación entre los máximos de $r^2$ y la ubicación de los centros de la QS3.
...
binwidth <- 10
regression[term == "amplitude"] %>% 
    # .[lat > -85] %>% 
    # .[, estimate.z := Anomaly(estimate), by = .(lat, month, term)] %>%
    ggplot(aes(lon, lat)) +
...
binwidth <- 10
ggplot(WrapCircular(regression[term == "amplitude" & 
                                   month %in% c(7,12)]), 
       aes(lon, lat)) +
    geom_contour_fill(aes(z = estimate), 
                      breaks = AnchorBreaks(0, binwidth, 0)) +
...
stream.reg.f <- memoise(function() {
    stream[, .(lon, lat, date, psi.z)] %>% 
        .[index[, .(date, amplitude, phase)], on = "date"] %>% 
        .[, FitLm(psi.z, amplitude = amplitude/sd(amplitude), phase, 
                  se = TRUE), 
          by = .(lat, lon, month(date))] %>% 
        .[term != "(Intercept)"] %>% 
        .[, t := abs(estimate)/std.error] %>% 
        .[, psi.z := estimate] %>% 
        .[, c("f.x", "f.y") := WaveFlux(.SD), by = .(term, month)] %>% 
        return()
})
stream.reg <- stream.reg.f()
...
ggplot(stream.reg[term == "amplitude" & lat <= 0], 
       aes(lon, lat)) +
    geom_contour_fill(aes(z = estimate*1e-6), 
                      breaks = AnchorBreaks(0, binwidth, 0)) +
    geom_vector(aes(dx = f.x, dy = f.y), skip = 4, min.mag = 0.005,
                arrow.length = 0.2, size = 0.2) +
    map.world +
    scale_s_map() +
    scale_mag(max = 0.05) +
    scale_fill_divergent(breaks = AnchorBreaks(0, binwidth, 0), 
...
speedy.f <- memoise(function() {
     rbindlist(lapply(seq_along(runs), function (x) {
        ...
        .[, t := t - 273.15] %>% 
        .[, c("k", "amplitude", "phase", "r2") := FitWave(gh, 3),
          by = .(lat, lev, date, run)] %>% 
        .[, QS3 := metR::FilterWave(gh, 3), by = .(lat, lev, date, run)] %>% 
        return()
})
speedy <- speedy.f()
...
#         .[, run := factor(run, levels = runs)] %>% 
#         .[, t := t - 273.15] %>% 
#         .[, c("amplitude", "phase", "r2") := FitWave(gh, 3),
#           by = .(lat, lev, date, run)] %>% 
...
speedy.mean.season <- speedy[, lapply(.SD, mean), 
                             by = .(lon, lat, lev, season(date, lang = "es"), run)]
...
Se realizó primero una corrida Control utilizando el modelo SPEEDY. La misma cubrió el período 1985-2014, con los modelos de suelo y hielo activados y considerando como forzante las medias mensuales de SST provenientes de la base de datos HadISST [@Rayner2003]. Se utilizó una atmósfera en reposo como condición inicial observando que el tiempo de *spin-up* es menor a un mes, por lo que se utilizaron todos los meses de simulación en el análisis.

Para la validación de la corrida Control se utilizaron los datos del NCEP, descriptos en la \autoref{datos-y-modelo} y utilizados en los Capítulos \ref{climatologia-observada} y \ref{onda-3}. La descripción se concentra en mostrar los resultados para diferentes variables en los niveles de 200hPa y 500hPa, como representativos de los niveles altos y medios de la troposfera respectivamente. Las conclusiones no cambian substancialmente en el resto de los niveles.
...
```{r ghz-sp-nc, fig.class = "fullpage", fig.cap = "Z* (mgp) (SPEEDY --Control-- sombreado, NCEP contornos)."}
...
ggplot(speedy.mean.season[run == "Control" & lev %in% plot.levs.sp], aes(lon, lat)) +
    ...
    geom_label_contour_back(aes(z = gh.z),
                            breaks = AnchorBreaks(0, binwidth, 0),
                            data = ncep.mean.season[lev %in% plot.levs.sp]) +
    scale_fill_divergent(breaks = AnchorBreaks(0, binwidth, 0),
                         name = "Z*", 
...
```{r ghz-sp-nc-corte60, fig.class = "pagewidth", fig.cap = "Corte zonal de Z* (mgp) en 60°S (SPEEDY --Control-- sombreado, NCEP contornos)."}
binwidth <- 20
ggplot(speedy.mean.season[run == "Control" & lat %~% cutlat], aes(lon, lev)) +
    ...
    geom_label_contour_back(aes(z = gh.z, label = ..level..), 
                             breaks = AnchorBreaks(0, binwidth, 0), skip = 2,
                            data = ncep.mean.season[lat %~% cutlat]) +
    scale_fill_divergent(breaks = AnchorBreaks(0, binwidth, 0),
                         name = "Z*",
                         guide = guide_colorstrip_bottom()) +
...
```{r ghz-dif-sp-nc, fig.class = "fullpage", fig.cap = "Diferencia de Z* (mgp) entre SPEEDY --Control-- y NCEP."}
binwidth <- 20
ggplot(sp.nc[lev %in% plot.levs.sp & variable == "gh.z"], aes(lon, lat)) +
    geom_contour_fill(aes(z = dif), breaks = AnchorBreaks(0, binwidth, 0)) +
    scale_fill_divergent(name = "Z*", breaks = AnchorBreaks(0, binwidth, 0),
                         guide = guide_colorstrip_bottom(45)) +
    scale_linetype(guide = "none") +
    facet_grid(season ~ lev, labeller = labeller(lev = lev.lab)) +
    map.SH +
...
```{r t-nc-sp, fig.cap = "Campo medio de temperatura (°C) (SPEEDY --Control-- sombreado, NCEP contornos).", fig.class = "fullpage"}
binwidth <- 5
cowplot::plot_grid({
    ggplot(speedy.mean.season[run == "Control" & lev %in% 200], 
           aes(lon, lat, z = t)) +
...
{
    ggplot(speedy.mean.season[run == "Control" & lev %in% 500],
           aes(lon, lat, z = t)) +
        geom_contour_fill(binwidth = binwidth) +
        # geom_contour_fine(aes(z = gh.z), binwidth = binwidth, exclude = 0,
        # color = "black", size = 0.2) +
...
{
    ggplot(speedy.mean.season[run == "Control" & lev %in% 850], 
           aes(lon, lat, z = t)) +
        geom_contour_fill(binwidth = binwidth) +
        # geom_contour_fine(aes(z = gh.z), binwidth = binwidth, exclude = 0,
        # color = "black", size = 0.2) +
        ...
        geom_label_contour_back(aes(label = ..level..),
                                binwidth = binwidth,
                                data = ncep.mean.season[lev %in% 850]) +
        stat_subset(aes(subset = lev > sp),
                    geom = "tile", color = "white", fill = "white") +
        # geom_hline(yintercept = cutlat,
        # linetype = 2, size = 0.5) +
        scale_fill_viridis_c(name = "Z*",
                             breaks = MakeBreaks(binwidth),
...
```{r tz-sp-nc, fig.cap = "Anomalía zonal de temperatura (°C) (SPEEDY --Control-- sombreado, NCEP contornos)", fig.class = "fullpage"}
...
binwidth <- 1
b <- function(x, binwidth) 1
ggplot(speedy.mean.season[run == "Control" & lev %in% plot.levs2], 
       aes(lon, lat, z = t.z)) +
    geom_contour_fill(breaks = AnchorBreaks(0, binwidth, 0)) +
    geom_contour2(aes(linetype = factor(sign(-..level..))),
                      breaks = AnchorBreaks(0, binwidth, 0),
                      data = tz.ncep) +
    geom_label_contour_back(aes(label = ..level..),
                            skip = 2,
...
```{r u-sp-nc-corte, fig.class = "pagewidth", fig.cap = "Media zonal del viento zonal (m/s) (SPEEDY --Control-- sombreado, NCEP contornos). La región blanca representa la topografía media."}
speedy.mean.season[run == "Control", 
    ...
    geom_contour_back(aes(z = u, linetype = factor(-sign(..level..))), 
                      ...
                      data = ncep.mean.season[, .(u = mean(u)), 
                                              by = .(lat, lev, season)]) +
    geom_label_contour_back(aes(z = u), 
                            breaks = AnchorBreaks(0, 5, 0),
                            data = ncep.mean.season[, .(u = mean(u)), 
                                                    by = .(lat, lev, season)]) +
    surface +
...
```{r etady-sp-nc, fig.class = "fullpage", fig.cap = "Gradiente meridional de vorticidad absoluta ($1\\times10^{11}(ms)^{-1}$) (SPEEDY --Control--). La línea negra marca regiones con valores negativos."}
mult <- 1e11
binwidth <- 1e-11*mult
ggplot(speedy.mean.season[run == "Control" & lev == 200 & lat > -85], 
       aes(lon, lat, z = eta.dy*mult)) +
    stat_contour_fill(breaks = AnchorBreaks(0, binwidth)) +
    geom_contour_fine(breaks = 0, color = "black") +
    map.SH +
    scale_fill_divergent(name = "Gradiente meridional de vorticiad absoluta", 
                         guide = guide_colorstrip_bottom(45),
...
```{r ks-sp,  include = FALSE, fig.cap = "Número de onda estacionario en 300hPa (SPEEDY --Control--).", fig.height = 6}
a <- 6371000
...
rbind(ncep.mean.season[lev == 200, .(lon, lat, lev, season, k, k.full)] %>% 
          .[lon %~% 180] %>% 
          .[, mod := "NCEP"], 
      sp.ks[lon %~% 180] %>% 
          .[, mod := "SPEEDY -Control-"]) %>%
...
```{r psi-sp, fig.class = "fullpage", fig.cap = "$\\psi$ media en 200hPa (contornos cada $2\\times10^{-7}m^2/s$), anomalía zonal de $\\psi$ (sombreado,  $0,25\\times10^{-7}m^2/s$) y flujos de actividad de onda medios. (SPEEDY --Control--)"}
...
```{r ampl-sp-nc, fig.class = "pagewidth", fig.cap = "Amplitud de la QS3 a partir de Fourier (mgp) (SPEEDY --Control-- sombreado, NCEP contornos)."}

# qs.sp.season <- speedy[, lapply(.SD, mean),
#                       by = .(lat, lev, season(date), run), 
#                       .SDcols = c("amplitude", "phase", "r2")]

qs.sp.season <- speedy.mean.season[, FitWave(gh, k = 3), by = .(lat, lev, run, season)]
...
ggplot(qs.sp.season[run == "Control"], aes(lat, lev)) +
    ...
    geom_label_contour_back(aes(z = amplitude), alpha = 1,
                            stroke.color = "#452F73", color = "white",
                            data = ncep.qs[k == 3], binwidth = binwidth) +
    surface +
    # geom_index.region(rect.annotation) +
    scale_fill_viridis_c(name = "Amplitud (escala logarítmica)", 
...
```{r qs3-sp-nc, fig.class = "pagewidth", fig.cap = "Z* reconstruida a partir de la QS3 (mgp) (SPEEDY --Control-- sombreado, NCEP contornos).", fig.height = 6}
binwidth <- 10
...
ggplot(qs.wave.sp.season[run == "Control"], aes(lon, lat)) +
    ...
    geom_contour_back(aes(z = mean, linetype = factor(-sign(..level..))),
                      binwidth = binwidth,
                      data = qs.wave.season[lev == 300]) +
    geom_label_contour_back(aes(z = mean), 
                            binwidth = binwidth,
                            data = qs.wave.season[lev == 300]) +
    map.SH +
    scale_s_map() +
...
speedy.mean.season.l <- melt(speedy.mean.season, 
                             id.vars = c("lon", "lat", "lev", "season", "run"))
speedy.mean.season.l[, value := as.numeric(value)]
...
ggplot(sp.dif[lev == 200], aes(lon, lat)) +
    geom_contour_fill(aes(z = u), breaks = AnchorBreaks(0, binwidth, 0)) +
    scale_fill_divergent(name = "Z*", 
                         breaks = AnchorBreaks(0, binwidth, 0),
                         guide = guide_colorstrip_bottom(45)) +
    scale_linetype(guide = "none") +
...
```{r psi-sp-runs, fig.class = "fullpage", fig.cap = "Anomalía zonal de $\\psi$ (sombreado,  $1\\times10^{-7}m^2/s$) y flujos de actividad de onda medios para cada corrida."}
mult <- 1e-7
binwidth <- 0.25
ggplot(psi.sp.season, aes(lon, lat)) +
    ...
    scale_fill_divergent(name = "Z*", 
                         breaks = AnchorBreaks(0, binwidth, 0),
                         guide = guide_colorstrip_bottom(45)) +
    geom_vector(aes(dx = f.lon, dy = f.lat), skip = 2,
                min.mag = 0.01, size = 0.2) +
    scale_mag(max = 0.2, label = "0.2") +
    scale_linetype(guide = "none") +
    map.world +
...
En la \autoref{fig:ampl-sp-runs} se muestra la amplitud de la QS3 (método AM) para cada estación y cada corrida.  Primavera es la estación con el cambio más dramático. Mientras que la señal es muy alta en la troposfera y latitudes medias para la corrida Control, es prácticamente despreciable en NOLAND y SSTZONAL. En verano se observa también una reducción de la señal de la QS3 en 60°S entre Control, NOLAND y SSTZONAL además de un ligero corrimiento hacia el sur. La señal de más al norte, en cambio, no presenta cambios importantes. En otoño la poca señal existente en latitudes medias en Control y NOLAND prácticamente desaparece en SSTZONAL pero se observa un aumento de la amplitud de la QS3 en la costa antártica. No obstante, la baja resolución del modelo hace cuestionable los resultados en la vecindad de una topografía tan compleja como la Antártida.

En invierno, NOLAND tiene la mayor señal de QS3 entre las simulaciones y Control, la mínima, con SSTZONAL presentando valores intermedios. Hay que tener en cuenta que la falta de representación de la estratósfera limita la simulación de las ondas estacionarias y en particular la QS3 en esta estación. En consecuencia, se requieren más estudios para evaluar y entender este resultado que superan los planeados en esta tesis.  
...
mean.qs.sp <- speedy[lon == 0, .(amplitude = mean(amplitude)), 
                     by = .(lat, lev, run, season(date, lang = "es"))] 
...
sp.index <- speedy[(lev %between% levs.index) & 
                       (lat %between% lats.index) & 
                       lon == 0,
                   .(amplitude   = mean(amplitude),
                     r2          = mean(r2),
...
sp.index[, .(sd = sd.circular(phase.c)*180/pi/3), by = .(run, season(date, lang = "es"))] %>% 
...
copy(sp.index) %>% 
    ...
    .[, interf := as.numeric(phase - meanphase)] %>% 
    .[, mean(abs(interf) %between% (c(1/3, 2/3)*120*pi/180))*100, by = .(run, season(date, lang = "es"))] %>%
    dcast(run ~ season) %>% 
    knitr::kable(caption = "Porcentaje de meses con interferencia destructiva - NO SE PUBLICA",
                 digits = 3)
```
...
A través de la aplicación del análisis de componentes principales se encontró que la estacionalidad de la QS3 no coincide exactamente con las estaciones climatológicas clásicas. Se concluyó que la división EFM, AM, JJ, ASO y ND representa una mejor clasificación. La misma resaltó que durante noviembre y diciembre la QS3 no presenta rasgos de onda estacionaria y permitió describir con mayor claridad la inclinación de la QS3 en la vertical durante junio y julio. Esto sugiere que la extensión vertical de la QS3 hasta la estratósfera se debe, al menos en parte, a la propagación vertical de ondas de Rossby desde la tropósfera.
