...
qs.eof.full <- EOF(fields[lev == 300], lon + lat ~ date, value.var = "QS3", n = 10)

qs.eof <- EOF(fields[lev == 300], lon + lat ~ date, value.var = "QS3", n = 2)
...
composition <- fields[date %in% select.dates,
                      .(gh.z = mean(gh.z)),
                      by = .(lon, lat, campo, month(date))]
binwidth <- 30
ggplot(composition[campo == "ncep"], aes(lon, lat)) +
    geom_contour_fine(aes(z = gh.z, color = ..level..),
                      binwidth = binwidth) +
    map.SH +
    scale_s_map() +
...
regression <- fields[campo == "ncep"][
    index[, .(date, amplitude, phase.mode)], on = "date"][
        , setNames(ExtractLm(RcppArmadillo::fastLm(cbind(mean = 1, 
                                                         amplitude, 
                                                         phase.mode),
                                                   gh.z)),
...
ggplot(regression[regressor == "amplitude"], aes(lon, lat)) +
    stat_contour(aes(z = estimate, color = ..level..), binwidth = 0.5) +
    map.SH +
    # scale_s_map() +
    scale_y_latitude(limits = c(-90, 0)) +
    scale_x_longitude() +
    scale_color_divergent(breaks = MakeBreaks(binwidth = 0.5), 
                          guide = guide_colorbar_bottom()) +
    coord_polar() +
    facet_wrap(~month, ncol = 3, nrow = 4, dir = "v",
               labeller = labeller(month = month.abb_sp))
