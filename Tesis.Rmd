...
```{r max_mean_compara, fig.show = "hold", fig.class = "textwidth", fig.cap = "Distribució de amplitud para 12 fechas. En rojo la amplitud máxima, en azul la amplitud media.", fig.subcap = c("Discribución", "Corte vertical."), fig.ncol = 1}
ncep.qs[, index.region := (lat %b% rect.annotation[1:2]) & 
            (lev %b% c(30, 925))]
max.mean <- ncep.qs[index.region == TRUE & k == 3, 
                    .(max.ampl = max(amplitude),
                      mean.ampl = mean(amplitude)), 
                    by = .(date)]
set.seed(44)
select.dates <- sample(ncep.qs$date, 12)
ggplot(ncep.qs[index.region == TRUE & k == 3 &
                   date %in% select.dates], aes(amplitude)) + 
    # geom_density() +
    geom_vline(aes(xintercept = max.ampl), color = "red", size = 0.4,
               data = max.mean[date %in% select.dates]) +
    geom_vline(aes(xintercept = mean.ampl), color = "blue",
               data = max.mean[date %in% select.dates]) +
...
M <- max(max.mean[date %in% select.dates, max.ampl])
binwidth <- 15
ggplot(ncep.qs[k == 3 &
                   date %in% select.dates], aes(lat, lev)) + 
    stat_contour_fill(aes(z = amplitude), binwidth = binwidth) +
    geom_index.region(rect.annotation) +
    geom_segment(data = max.mean[date %in% select.dates], inherit.aes = F,
                 size = 1, color = "red", 
                 aes(x = -90, xend = -90, y = 1000, 
                     yend = 10^(-2/M*max.ampl + 3))) +
    geom_segment(data = max.mean[date %in% select.dates], inherit.aes = F,
                 size = 1, color = "blue", 
                 aes(x = -90, xend = -90, y = 1000, 
                     yend = 10^(-2/M*mean.ampl + 3))) +
    surface +
...
```{r campos_anomalia_select, fig.class = "fullwidth", fig.cap = "Anomalía zonal geopotencial en 300hPa para fechas seleccionadas."}
