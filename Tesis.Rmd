...
```{r k-steady, fig.cap = "Número de onda estacionario en 300hPa.", fig.height = 6}
a <- 6371000

ks <- ncep.mean.season[lev == 300, .(k = sqrt(eta.dy/u)*(a*cos(lat*pi/180))), 
...
ggplot(ks, aes(lon, lat)) +
    ...
    geom_tile(data = ks[k.full == -1], fill = "white", color = "white") +
    map.SH +
    scale_s_map() +
...
```{r ks-lat, fig.class = "textwidth", fig.cap = "Número de onda estacionario medio por círculo de latitud."}
...
WaveFlux <- function(psi, p = 250, a = 6371000) {
    ...
    psi[, c("psi.dlon", "psi.dlat") := Derivate(psi.z ~ lon + lat, 
                                              bc = c("cyclic", "none"))] %>% 
        .[, psi.ddlon := Derivate(psi.z ~ lon, bc = "cyclic", order = 2), 
          by = lat] %>%
    ...
    list(f.lat = psi$f.lat, f.lon = psi$f.lon)
}
```
...
stream.mean.season[, c("f.lat", "f.lon") := WaveFlux(.SD), by = season]
```
...
ggplot(stream.mean.season, aes(lon, lat)) +
    ...
    geom_arrow(aes(dx = f.lon, dy = f.lat), skip = 3, min.mag = 0.05, 
               arrow.size = 0.2, scale = 50) +
    map.SH +
    scale_s_map() +
...
O, medio equivalente, usar la correlación entre meses:

```{r cros-cor, fig.cap = "Matriz de Correlación del campo de QS3."}
qs.field <- fields[lev == 300, .(lon, lat, date, QS3)]
qs.croscor <- qs.field[, {value = QS3
qs.field[, cor(value, QS3), by = .(month2 = month(date))]}, 
by = .(month1 = month(date))]

ggplot(qs.croscor[month1 != month2], aes(month1, month2)) +
    geom_point(aes(color = V1, size = V1^2)) +
    scale_x_continuous(breaks = 1:12, labels = month.abb_sp) +
    scale_y_continuous(breaks = 1:12, labels = month.abb_sp) +
    scale_color_divergent()
```
...
cor.months.z <- fields[lat > -90 & lev == 300, 
                       .(corelation = cor(gh.z, QS3)), 
                       by = .(lon, lat, month(date))][
                           , cor.w := corelation*sqrt(cos(lat*pi/180))]

binwidth <- 0.15
ggplot(cor.months.z[, .(cor = mean(corelation^2)), 
    ...
    facet_wrap(~season)
# 
# cor.eof.z <- EOF(cor.months.z, lon + lat ~ month, value.var = "corelation", n = 4)
# 
# ggplot(cor.eof.z$right, aes(PC, value)) + 
#     geom_point(aes(color = season(month)))
# 
# ggplot(cor.eof.z$left, aes(lon, lat)) +
...
# cor.eof <- EOF(cor.months, lon + lat ~ month, value.var = "corelation", n = 4)
# 
# 
# ggplot(cor.eof$right, aes(PC, value)) + 
#     geom_point(aes(color = season(month)))
# 
# ggplot(cor.eof$left, aes(lon, lat)) +
#     stat_contour_fill(aes(z = value)) +
#     map.SH +
#     scale_fill_divergent() +
#     facet_wrap(~PC) 
...
```{r cor-monson, fig.cap = "Correlación cuadrada media para estaciones según Monsón de Sudamérica."}

binwidth <- 0.15
ggplot(cor.months.z[, .(cor = mean(corelation^2)), 
                    by = .(lon, lat, season = (month %in% 5:9))], 
       aes(lon, lat)) +
    stat_contour(aes(z = cor, color = ..level..), binwidth = binwidth) +
    map.SH +
    scale_color_viridis_c(breaks = MakeBreaks(binwidth),
    ...
    coord_quickmap() +
    facet_wrap(~season, ncol = 2,
               labeller = labeller(season = setNames(c("May-Sep", "Oct-Abr"),
                                                     c(TRUE, FALSE))))
```
...
Al igual que antes, ¿qué pasa si hago PSA?

```{r eof-com0, cache = FALSE}
eofs <- copy(fields[date %in% select.dates & campo == "ncep"]) %>% 
    .[, `:=`(year = year(date),
             gh.w = gh*sqrt(cos(lat*pi/(180))))] %>% 
    .[, EOF(.SD, lon + lat ~ year, value.var = "gh", n = 2:4)$left,
      by = .(campo, month(date))]

g <- ggplot(eofs[campo == "ncep"], aes(lon, lat)) +
    stat_contour_fill(aes(z = value), exclude = 0, complete = F, 
                      binwidth = 0.01) +
    map.SH +
    scale_fill_divergent() + 
    coord_quickmap() 
...
```{r eof-comp1, fig.class = "fullpage", fig.cap = "Campos EOF para cada mes (probablemente basura)"}
g + facet_grid_paginate(PC~month, ncol = 4, nrow = 3, page = 1,
               labeller = labeller(month = month.abb_sp))
```

```{r eof-comp2, fig.class = "fullpage", fig.cap = "Campos EOF para cada mes (probablemente basura)"}
g + facet_grid_paginate(PC~month, ncol = 4, nrow = 3, page = 2,
               labeller = labeller(month = month.abb_sp))
```

```{r eof-comp3, fig.class = "fullpage", fig.cap = "Campos EOF para cada mes (probablemente basura)"}
g + facet_grid_paginate(PC~month, ncol = 4, nrow = 3, page = 3,
               labeller = labeller(month = month.abb_sp))
```
...
regression <- ncep[lev == 300, .(lon, lat, date, gh)][
    index[, .(date, amplitude, phase.mode)], on = "date"][
        , setNames(ExtractLm(
...
date.max <- index[, date[which.max(abs(lon - phase*180/pi))]]
date.select <- as.Date("2013-06-01")
dates <- c(date.select, date.max)
...
ggplot(fields[campo == "qs3" & date %in% dates], 
       aes(lon, lat)) +
    stat_contour_fill(aes(z = gh), exclude = 0, complete = F,
                      binwidth = 10) +
...
```{r boxplot-fase-ncep2, fig.cap = "Ciclo anual de la fase (todos)"}
lat.lims <- c(-70, -22)
lon.lims <- c(230, 360)
map.arg <- geom_map2(BuildMap(countries = T)[lat %b% lat.lims & 
                                                 long %b% lon.lims])
