...
qs.wave.season[, 
    ...
    xtable::xtable(col.names = c("Estación", "[u*v*]"), digits = 3, caption = "Transporte meridinoal de cantidad de viento zonal NO SE PUBLICA") %>% 
    print(tabular.environment = "wraptable")
```
...
\todo[inline]{Fijarse acá (en el código), ver el tema de usar el paquete circular para la fase.}
...
El ciclo anual de la amplitud media se muestra en la \todo{fijarse que autoref no anda con subfig} \autoref{fig:ampl-ts1} donde los puntos son datos puntuales. \todo{hasta acá}  
