...
ggplot(RepeatLon(ncep.mean[lev == 850]), aes(lon, lat)) +
    ...
    geom_contour(aes(z = v, color = ..level..)) +
    map.SH.3 +
    scale_x_longitude(limits = c(0, 360)) +
    scale_y_continuous(limits = c(-90, 0), name = "") +
...
```{r}
ggplot(RepeatLon(ncep.mean[lev == 300 & lat < 0]), aes(lon, lat)) +
...
qs <- ncep[, FitQsWave(gh, n = 1:3), by = .(lat, lev, year)][!is.na(r2)]
qs.mean <- qs[, lapply(.SD, mean), by = .(lat, lev, k)][, year := NULL]
```

```{r}
ggplot(qs.mean[lat < 0], aes(lat, lev)) +
    stat_fill_contour(aes(z = amplitude)) +
    geom_contour(aes(z = r2, color = ..level..)) +
