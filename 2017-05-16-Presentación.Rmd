...
```{r}
gdata <- field[, .(correlation = cor(gh[field == "qs"], gh[field == "ncep"])), 
               by = .(year(date), month(date))]
gdata2 <-  field[,
                 .(correlation = cor(gh[field == "qs"], gh[field == "ncep"])), 
                 by = .(month(date))] %>%
    .[, year := 2018]

ggplot(gdata, aes(year, as.factor(month), size = correlation, color = correlation)) +
    geom_point() + 
    geom_point(data = qs_index[select.amplitude == T, 
                               .(year = year(date), month = month(date))],
               inherit.aes = F, aes(year, as.factor(month)), 
