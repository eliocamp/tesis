...
ncfile <- nc_open("NCEP/air.mon.mean_sub.nc")
...
ncfile <- nc_open("NCEP/uwnd.mon.mean_sub.nc")
...
ncfile <- nc_open("NCEP/vwnd.mon.mean_sub.nc")
...
# guardo en disco
# fwrite(hgt, "NCEP/hgt_ncep.csv")
# fwrite(temp, "NCEP/temp_ncep.csv")
# fwrite(U, "NCEP/U_ncep.csv")
# hgt <- fread("NCEP/todo_ncep.csv")
# hgt2 <- fread("NCEP/todo_ncep.csv")

hgt[, year := stringi::stri_sub(time, from = 1, to = 4)]
hgt[, month := stringi::stri_sub(time, from = 6, to = 7)]
ncep <- hgt
fwrite(hgt, "NCEP/todo_ncep.csv")
save(ncep, file = "NCEP/todo_ncep.rda")
...
fwrite(m_ncep, "NCEP/todo_m_ncep.csv")
save(m_ncep, file = "NCEP/todo_m_ncep.rda")
...
save(olr, file = "NCEP/olr_todo.rda")
fwrite(olr, file = "NCEP/olr_todo.csv")

remove(olr, olr_small)

# SST

file <- "NCEP/sst_sub.nc"
ncfile <- nc_open(file)
lat <- ncvar_get(ncfile, "lat")
lon <- ncvar_get(ncfile, "lon")
time <- ncvar_get(ncfile, "time")
...
sst <- setDT(melt(sst_small, value.name = "sst", 
                  varnames = c("lon", "lat", "time")))
save(sst, file = "NCEP/sst_todo.rda")
fwrite(sst, file = "NCEP/sst_todo.csv")
